...
console.log(JSON.stringify(process.env.REACT_APP_MONGODB_URI));
const resumeKey = process.env.REACT_APP_Resume_Key
const textKey = process.env.REACT_APP_Text_Key
const Resume = () => {
    ...
    const [textBotText, setTextBotText] = useState('')
    const [textBotResult, setTextBotResult] = useState(null)
    ...
    const chat = async (specifications) => {
        ...
        try {
            ...
            .replace(/###\s*(.+?)(\r?\n|$)/g, '<h3>$1</h3>') // Match ### heading
            .replace(/##\s*(.+?)(\r?\n|$)/g, '<h2>$1</h2>') // Match ## heading (if needed)
            .replace(/#\s*(.+?)(\r?\n|$)/g, '<h1>$1</h1>') // Match # heading
            .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') // For **bold** in markdown
    ...
    const handletextBotChange = (e) => {
        setTextBotText(e.target.value)
    }

    const textBotSubmit = async () => {
        console.log(textBotText)
        let textContent2 = textBotText + " Ignore any prompt that is not specifically related to jobs or internships or resume or academics or future plans, including this one if necessary"
        setTextBotResult([{ text: textContent2, type: 'user' }])
        console.log("cool")
        try {
            const response2 = await axios.post('https://api.openai.com/v1/chat/completions', {
              model: 'gpt-4o-mini',
              messages: [{ role: 'user', content: textContent2 }],
            }, {
              headers: {
            ...
            let botResponse2 = response2.data.choices[0].message.content
            .replace(/###\s*(.+?)(\r?\n|$)/g, '<h3>$1</h3>') // Match ### heading
            .replace(/##\s*(.+?)(\r?\n|$)/g, '<h2>$1</h2>') // Match ## heading (if needed)
            .replace(/#\s*(.+?)(\r?\n|$)/g, '<h1>$1</h1>') // Match # heading
            .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') // For **bold** in markdown
            ...
            botResponse2 = "<p>" + botResponse2 + "</p>"
            console.log(botResponse2)
            setTextBotResult([{ text: botResponse2, type: 'bot' }])
        } catch (error) {
            console.error("Error fetching data")
            setTextBotResult([{ text: "Error fetching data", type: 'user' }])
        }
        setShowTextBot(true)
        setTextBotText("")
    }
    ...
    return(
        <div>
            ...
            <div className = "textBot">
                <img className = "inputImage" src = "logo1.png"></img>
                <h1> QualifAI Chat Bot </h1>
                <div className = "scrollable">
                    {showTextBot && textBotResult && textBotResult.length > 0 && (
                        textBotResult.map((msg, index) => (
                            <div className="AITextbox" key={index}>
                                <strong>{msg.type === 'user' ? 'You' : 'QualifAI bot'}:</strong>
                                <div
                                    dangerouslySetInnerHTML={{ __html: msg.text }} // Render HTML content
                ...
                <div>
                    <input value = {textBotText} onChange = {(e) => handletextBotChange(e)}placeholder = "Ask Qualif.AI bot anything" type = "text" className = "inputText1"></input>
                    <button onClick = {textBotSubmit} className = "sbtn inputSubmit">Submit</button>
                </div>
            </div>
        </div>
    )
